...
#------------------------------------ MSE TextBox ----------------------------------------------------------------------
MSE=Entry(Master)
MSE.place(x=600,y=144)
L5=Label(Master,text="MSE")
L5.place(x=780,y=145)
L5.configure(font='helvetica 12 bold ')
#------------------------------------ MSE TextBox ----------------------------------------------------------------------
#------------------------------------ Bias Check Box ----------------------------------------------------------------------
bais_checked=IntVar()
Checkbutton(Master,text="Bais",font='helvetica 12 bold ',variable=bais_checked).place(x=600, y=172)
#------------------------------------ Bias Check Box ----------------------------------------------------------------------

#------------------------------------ TRAIN BUTTON -----------------------------------------------------------------------
def Train_test_button(features, classes, Rate, epochs, mse, bais):
    global W, b, loss, X_train, X_test, y_train, y_test, y2_pred
    W, b, loss, X_train, X_test, y_train,y_test,y2_pred=Algorithms.train_Test_NN(features, classes, Rate, epochs,mse, bais)

Button(Master,text="Train",font='helvetica 8 bold ',bg='blue',height=3,width=30,command=lambda  :Train_test_button(F_menuclicked.get(),C_menuclicked.get(),Rate.get(),Epoch.get(),MSE.get(),bais_checked.get())).place(x=670,y=240)
#------------------------------------ TRAIN BUTTON -----------------------------------------------------------------------

#------------------------------------ DRAW LINE BUTTON -----------------------------------------------------------------------
def draw_line_button():
    Algorithms.draw_line(W, b, X_test)

Button(Master, text="Draw Line", font='helvetica 8 bold ', bg='blue', command=lambda: draw_line_button(),height=3, width=30).place(x=450, y=240)
#------------------------------------ DRAW LINE BUTTON -----------------------------------------------------------------------

#------------------------------------ EVALUATE BUTTON -----------------------------------------------------------------------
def Evaluate_button():
    CM, acc = Algorithms.evaluate_NN(y2_pred, y_test)
    acc = acc * 100
...
Button(Master, text="Evaluate", font='helvetica 8 bold ', bg='blue', height=3, width=30,command=lambda: Evaluate_button()).place(x=230, y=240)
#------------------------------------ EVALUATE BUTTON -----------------------------------------------------------------------
